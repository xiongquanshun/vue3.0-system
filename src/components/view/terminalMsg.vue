            ...
            <div class="form-boxs">
                <el-form ref="form" :model="form" label-width="80px">
                    ...
                    <el-form-item label="归属机构">
                        <el-input v-model="form.ascription" class='el-input'></el-input>
                    </el-form-item>
                    <el-form-item label="终端编码">
                        <el-input v-model="form.address" class='el-input'></el-input>
                        <font style="color:#409EFF;margin-left:10px;cursor:pointer" @click="randomNum">随机生成</font>
                    </el-form-item>
                   
                    <el-form-item label="设放地址" class='addr-select' >
                        <el-select @change="choseProvince" placeholder="省级地区" v-model="provinceText">
                            <el-option v-for="(item,index) in province" :key='index' :label="item" :value="item"></el-option>
                        </el-select>
                        <el-select @change="choseCity" placeholder="市级地区" v-model="cityText">
                            <el-option v-for="(item,index) in city" :key='index' :label="item" :value="item"></el-option>
                        </el-select>
                        <el-select @change="choseBlock" placeholder="区级地区" v-model="countyText">
                            <el-option v-for="(item,index) in county" :key='index' :label="item" :value="item"></el-option>
                        </el-select>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            /* -----------------城市级联--------------- */
            choseProvince(e){
                const {province,cityList}=this;
                let keys='';
                // 重置市、县
                this.provinceText=e;this.countyText='';this.cityText=''
               ...
               this.city=this.filterObj(cityList[0],[keys])[keys]
                let act_addr="广东省深圳市福田区会展中心"; //活动地址
                this.mapSearch(act_addr);
            },
            choseCity(e){
                const {city,cityList}=this;
                console.log(this.city)
                // 重置市、县
                this.countyText='';
                this.cityText=e;this.county={};
                console.log("选择市")
            },
            choseBlock(e){
                this.countyText=e;
            ...
            mapInit(){
                ...
                this.map=map;
                let act_addr="广东省深圳市福田区深茂商业大厦17c"; //活动地址
                this.mapSearch(act_addr);
            },
            // 搜寻指定位置
            mapSearch(act_addr){
                ...
                AMap.service(["AMap.PlaceSearch"], function() {
                    ...
                    //关键字查询
                    placeSearch.search(act_addr, function(status, result) {
                        if(status=='no_data'){
                        // Toast({
                        //     message: '查询不到相应活动地址',
                        //     duration: 1000
...
<style>
    .amap-logo,.amap-copyright{display: none !important}     /* 高德地图 */
    .terminalMsg th{background: rgb(237,242,246)}
    .terminalMsg .cell{text-align: center}
    .terminalMsg  .addr-select .el-form-item__content{display:flex;}
</style>

<style scoped>
    .terminalMsg{display: flex;justify-content:center;}
